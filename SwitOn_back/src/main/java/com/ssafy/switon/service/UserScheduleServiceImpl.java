...
public class UserScheduleServiceImpl implements UserScheduleService {
	...
	public boolean changeStatus(int userId, int scheduleId, int status) {
		...
		if((originalStatus & (1 << (status - 1))) == 0) {
			...
			if(userSchedule.getStatus() == 3 || userSchedule.getStatus() == 7) {
	...
	public UserRate getUserParticipateRate(int userId, int studyId) {
		...
		double partMine = (double) part.getMine();
		double partTotal = (double) part.getTotal();
		double attendMine = (double) attend.getMine();
		double attendTotal = (double) attend.getTotal();
		if(part.getTotal() != 0) {
			participation = partMine / partTotal;
		}
		if(attend.getTotal() != 0) {
			attendance = attendMine / attendTotal;
		}
